...
public class SGMenu implements InventoryHolder {
    ...
    private final Map<Integer, SGButton> itemsbut;
    ...
    public void setButtonbut(int slot, boolean flag,  SGButton button) {
         itemsbut.put(slot, button);
    }
    /**
     * Adds the provided {@link SGButton} at the position denoted by the
    ...
    public SGButton getButton2(int slot) {
        if (itemsbut.containsKey(slot)) {
            return itemsbut.get(slot);
        } 
        return null;
    }
     ...
     * @return The {@link SGButton} that was in that slot or null if the slot was invalid or if there was no button that slot.
     */
    public SGButton getButton2(int page, int slot) {
        return getButton2((page * getPageSize()) + slot);
    }
    /// PAGINATION ///
    ...
    public Inventory getInventory() {
        ...
        if (itemsbut.size() > 0) {
            ...
            for (int i = pageSiz; i < pageSiz + 9; i++) {
                if (itemsbut.containsKey(i)) {
                    inventory.setItem(i, itemsbut.get(i).getIcon());
                }
            }
        }
